...
int main(int argc, char* argv[])
{
   ...
   input = imread("/home/pranay/Final_Year_Project/code/test3.jpg", CV_LOAD_IMAGE_COLOR);
   ...
   std::vector<int> y(input.size().height);
   for (int i = 0; i < input.size().height; ++i)
   {
    ...
    inputQuad[0] = Point2f( 0.43 * input.size().width,0.65 * input.size().height);
    inputQuad[1] = Point2f( 0.58 * input.size().width,0.65 * input.size().height);
    inputQuad[2] = Point2f( 0.1 * input.size().width,1 * input.size().height);
    inputQuad[3] = Point2f( 1 * input.size().width,1 * input.size().height);  
    ...
    circle(input, Point2f(0.43 * input.size().width,0.65 * input.size().height), 5, cv::Scalar(0, 0, 255), 1 , 8, 0);
    circle(input, Point2f(0.58 * input.size().width,0.65 * input.size().height), 5, cv::Scalar(0, 0, 255), 1 , 8, 0);
    circle(input, Point2f( 0.1 * input.size().width,1 * input.size().height), 5, cv::Scalar(0, 0, 255), 1 , 8, 0);
    circle(input, Point2f(1 * input.size().width,1 * input.size().height), 5, cv::Scalar(0, 0, 255), 1 , 8, 0);
    ...
    threshold(output_gray, gray_threshold, 170, 255, THRESH_BINARY);
    ...
    cv::reduce(gray_threshold, columnSum, 0, CV_REDUCE_SUM, CV_32S);
...
double ym_per_pix = 30.5/720; // meters per pixel in y dimension
double xm_per_pix = 3.7/720; // meters per pixel in x dimension
...
//generate y values using  coefficients of left region
// y = a*x^2 + b*x + c
