...
int main(int argc, char* argv[])
{
  ...
  const float* histRange = {range};
    ...
    //int channels[] = {0};
    calcHist( &grad, 1, 0, Mat(), grey_histogram, 1, &histSize, &histRange, uniform, accumulate);

    // Draw the histograms for B, G and R
    int hist_w = 512; int hist_h = 400;
    int bin_w = cvRound( (double) hist_w/histSize );
    Mat histImage( hist_h, hist_w, CV_8UC3, Scalar( 0,0,0) );
    normalize(grey_histogram, grey_histogram, 0, histImage.rows, NORM_MINMAX, -1, Mat() );

      /// Draw for each channel
  for( int i = 1; i < histSize; i++ )
  {
      line( histImage, Point( bin_w*(i-1), hist_h - cvRound(grey_histogram.at<float>(i-1)) ) ,
                       Point( bin_w*(i), hist_h - cvRound(grey_histogram.at<float>(i)) ),
